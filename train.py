...
if __name__ == "__main__":
    ...
    processor = SamProcessor.from_pretrained("facebook/sam-vit-base")
    seg_loss = monai.losses.DiceCELoss(sigmoid=True, squared_pred=True, reduction='mean') 
    #dice_coefficient= DiceMetric(include_background=True, reduction="mean")
    
    root_dir= "/home3/lsiblini/output/try/"
    filename="model_try.pt"
    train_output_path= "/home3/lsiblini/output/try/train_epoch_try.txt"
    val_output_path= "/home3/lsiblini/output/try/val_epoch_try.txt"
    ...
    json_list = "/home3/lsiblini/data/IRCAD/trainval_folds.json"
    
    tr, val = datafold_read(json_list, data_dir, key="training")

    train_dataset= SAMDataset(data_list= tr, processor=processor)
    train_dataloader = DataLoader(train_dataset, batch_size=10, collate_fn=pad_collate_fn, shuffle=True)#collate_fn=pad_collate_fn
    ...
    plt.savefig("/home3/lsiblini/output/try/curves_plot_try.png")
